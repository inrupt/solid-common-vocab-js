...
describe("LIT utils tests", () => {
  ...
  describe("WebID generation", () => {
    it("No environment value should generate with default", () => {
      ...
      expect(webId.value).to.include(
        "https://",
        username,
        LitUtil.DEFAULT_WEDID_SERVER_DOMAIN,
        "#"
      );
    });
  ...
  describe("ID generation", () => {
    ...
    it("Should throw if attempting to generate well known IRI without code context", () => {
      expect(() => LitUtil.generateWellKnownIri()).to.throw(
        "we require a code context"
      );
    });
    ...
    it("Should generate correct strings (for production ENV!)", () => {
      ...
      expect(LitUtil.generateWellKnownIri("hint").toString())
        .to.include(LitUtil.LIT_CORE_NAMESPACE)
        .and.to.include("/hint/");
    });
    ...
    it("Should generate correct strings (for development ENV!)", () => {
      ...
      expect(LitUtil.generateWellKnownIri("hint").toString()).to.include(
        "/hint/"
      );
    });
    ...
    it("Should generate code context", () => {
      ...
      expect(LitUtil.codeContext(new String())).equals(
        `${LitUtil.NO_APPLICATION_HINT}/String`
      );
      expect(LitUtil.codeContext(new String(), "func")).equals(
        `${LitUtil.NO_APPLICATION_HINT}/String/func`
      );
    });
  ...
  describe("RDF file tests", () => {
    it("Should load dataset", (done) => {
      LitUtil.loadTurtleFile("test/resources/list-users.ttl", function (data) {
        ...
        LitUtil.loadTurtleFileIntoDataset(
          "test/resources/list-users.ttl",
          rdf.dataset(),
          function (data) {
            expect(data.length).to.equal(41);
            done();
    ...
    it("Should load dataset using promise", (done) => {
      LitUtil.loadTurtleFilePromise("test/resources/list-users.ttl").then(
        function (data) {
          expect(data.length).to.equal(41);

          LitUtil.loadTurtleFileIntoDatasetPromise(
            "test/resources/list-users.ttl",
            rdf.dataset()
          ).then(function (data) {
            expect(data.length).to.equal(41);
    ...
    it("Should write dataset", () => {
      ...
      LitUtil.saveDatasetToFile(
        dataset,
        "test/resources/output/rdf-write-file-test.ttl"
      );
    });
  });
  ...
  describe("Converting to string", () => {
    it("Should write dataset to string", () => {
      const dataset = rdf
        .dataset()
        .add(rdf.quad(alice, RDF.type, SCHEMA.Person));
      LitUtil.datasetToString(dataset).then((result) => {
        expect(result).to.equal(
          `<${aliceIriAsString}> <${RDF.type}> <${SCHEMA.Person.value}> .\n`
        );
      });
    });

    it("Should write quad to string", async () => {
      const result = await LitUtil.quadsToString([
        rdf.quad(alice, RDF.type, SCHEMA.Person),
      ]);
      expect(result).to.equal(
        `<${aliceIriAsString}> <${RDF.type}> <${SCHEMA.Person.value}> .\n`
    ...
    it("Should write quads to console log", async () => {
      let result = await LitUtil.console(
        rdf.quad(alice, RDF.type, SCHEMA.Person)
      );
      expect(result).to.equal(
        `<${aliceIriAsString}> <${RDF.type}> <${SCHEMA.Person.value}> .\n`
  ...
  describe("isHttpOk", () => {
    ...
    it("Should throw on incorrect types", () => {
      expect(() => LitUtil.isHttpOk(false)).to.throw(
        "Could not determine datatype"
      );
      expect(() => LitUtil.isHttpOk()).to.throw("Could not determine datatype");

      expect(() => LitUtil.isHttpOk("whatever")).to.throw("Could not convert");

      expect(() => LitUtil.isHttpOk({})).to.throw(
        "we only support RDF Literals"
      );
      expect(() => LitUtil.isHttpOk(rdf.namedNode("https://200"))).to.throw(
        "we only support RDF Literals"
      );
    });
  });
  ...
  describe("replacing local name", () => {
    it("Should replace correctly", () => {
      expect(
        LitUtil.replaceIriLocalName(alice, "coreProfile").value
      ).to.be.equal("https://alice.example.org/profile/coreProfile");
      expect(
        LitUtil.replaceIriLocalName(alice, "extension/medical").value
      ).to.be.equal("https://alice.example.org/profile/extension/medical");
    });

    it("Should throw if no separator", () => {
      expect(() =>
        LitUtil.replaceIriLocalName(
          rdf.namedNode("https:No_hash_or_backslash"),
          "doesn't matter"
        )
      ).to.throw("Could not" + " find an IRI separator");
    });
  });
  ...
  describe("creating relative path", () => {
    it("Should return original if relative part undefined", () => {
      expect(
        LitUtil.makeRelativeIri(rdf.namedNode("http://example.com/whatever#me"))
          .value
      ).to.equal("http://example.com/whatever#me");
    });

    it("Should create correctly", () => {
      expect(
        LitUtil.makeRelativeIri(
          rdf.namedNode("http://example.com/whatever#me"),
          "newPart"
        ).value
      ).to.equal("http://example.com/whatever/newPart");
      expect(
        LitUtil.makeRelativeIri(
          rdf.namedNode("http://example.com/whatever/me"),
          "newPart"
      ...
      expect(
        LitUtil.makeRelativeIri(
          rdf.namedNode("http://example.com/whatever/a/b/c"),
          "../../newPart"
        ).value
      ).to.equal("http://example.com/whatever/newPart");
      expect(() =>
    ...
    it("Should also append GUID", () => {
      ...
      ).to.contain("http://example.com/whatever/newPart/");
      expect(
        LitUtil.makeRelativeIri(
          rdf.namedNode("http://example.com/whatever/me"),
          "newPart",
  ...
  describe("replace all", () => {
    it("Should replace all", () => {
      expect(LitUtil.replaceAll("test user name", " ", "_")).to.equal(
        "test_user_name"
      );
    });
  });
  ...
  describe("validate IRI", () => {
    it("Should succeed with HTTP strings or named nodes", () => {
      expect(
        LitUtil.validateIri("http://example.com/test", "Subject", {})
      ).to.deep.equal(rdf.namedNode("http://example.com/test"));
      expect(
        LitUtil.validateIri("https://example.com/test", "Subject", {})
      ).to.deep.equal(rdf.namedNode("https://example.com/test"));
      expect(
        LitUtil.validateIri(
          rdf.namedNode("http://example.com/test"),
          "Subject",
          {}
        )
      ).to.deep.equal(rdf.namedNode("http://example.com/test"));
      expect(
        LitUtil.validateIri(
          rdf.namedNode("https://example.com/test"),
          "Subject",
          {}
        )
      ).to.deep.equal(rdf.namedNode("https://example.com/test"));
    ...
    it("Should fail with non-HTTP strings", () => {
      ...
      expect(() =>
        LitUtil.validateIri("example.com/test", "Subject", litObject)
      ).to.throw("Subject", "test state", "example.com/test");
      expect(() => LitUtil.validateIri(null, "Predicate", litObject)).to.throw(
        "Predicate",
        "null",
        "example.com/test"
      );
  ...
  describe("mismatching IRIs", () => {
    it("Should not explain if completely different", () => {
      ...
      expect(result).to.include.all.string(
        first.toString(),
        second.toString(),
        "*must* be the same."
      );
  ...
  describe("HTTP header tests", () => {
    it("Should use our LIT-specific prefixes", () => {
      expect(LitUtil.prefixForHttpHeader()).to.startsWith(
        LitUtil.LIT_CORE_NAMESPACE
      );
    });
  });
  ...
  describe("Dataset util function tests", () => {
    it("Should read the first value from a given dataset", () => {
      ...
      expect(LitUtil.firstDatasetValue(emptyDataset, "default value")).to.equal(
        "default value"
      );
    });
  });
