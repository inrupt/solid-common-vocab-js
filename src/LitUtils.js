...
const LitContextError = require('./LitContextError')
...
// For use when generating IRI's but the programmer has not configured a specific application name (just to be used
// as a hint within IRI's generated by that application).
module.exports.NO_APPLICATION_HINT = 'NO-APP'

// NOTE: This namespace is defined in the LIT Core RDF vocabulary, but we can't have a dependency on the source code
// generated from that RDF as we are too low-level a library. Unfortunately this means we need to manually keep this
// value in sync with that RDF vocabulary (and the code generated from it).
// TODO: Currently (Mar 2019) we have very limited only use of this value, so perhaps we should force the passing of
// this value to that GUID-generating method, or force the use of an environment variable...
const litCoreNamespace = 'https://w3id.org/lit/vocab/core#'
module.exports.LIT_CORE_NAMESPACE = litCoreNamespace

/**
 * When adding HTTP header data to an RDF payload we need to provide full IRI's, so we use this very LIT-specific
 * prefix for them, e.g. for passing back an 'accept-language' value that ultimately we want added to an HTTP response
 * as the standard HTTP 'accept-language' header.
 *
 * @returns {string}
 */
...
/**
* Creates a WebID based on the specified username (which is used as a subdomain).
* The IRI generated depends on how the system is configured (i.e. to use a Solid server or just local development).
*
* @param userName
 ...
 *
 * @param codeContext Optional value to include in the IRI (intended to provide a helpful reference to where in our
 * code this IRI is being generated, i.e. the context within our code).
 * @returns {*}
 */
...
/**
 * Provide a context string from the current location in the code (i.e. using the specified class and function name).
 * This is intended to aid debugging, by providing a little more context to IRI's generated throughout the code (e.g.
 * this context might tell you where in the code a collection of RDF was created).
 *
 * ISSUE: Security might be a concern in terms of leaking code internals externally?
...
module.exports.codeContext = (clazz, func) => {
  let iri = process.env.IRI_HINT_APPLICATION ? `${process.env.IRI_HINT_APPLICATION}` : module.exports.NO_APPLICATION_HINT
...
/**
 * Loads a Turtle file (just 'cos we use Turtle by default - easy to refactor if other serializations needed) and
...
module.exports.loadTurtleFile = (filename, resolveFunc, rejectFunc) => {
  return this.loadTurtleFileIntoDataset(filename, undefined, resolveFunc, rejectFunc)
}
...
/**
 * Loads a Turtle file (just 'cos we use Turtle by default - easy to refactor if other serializations needed) into
 * the specified dataset and also returns the parsed dataset.
 *
 * @param filename The Turtle resource to load.
 * @param dataset Dataset to put loaded data (if 'undefined' we'll create an empty dataset).
 * @param resolveFunc
 * @param rejectFunc
 */
module.exports.loadTurtleFileIntoDataset = async (filename, dataset, resolveFunc, rejectFunc) => {
  // const mimeType = 'text/turtle'
  // const data = fs.readFileSync(filename, 'utf8')
  //
  // const parser = rdfFormats.parsers[ mimeType ]
  // const quadStream = parser.import(stringToStream(setData))
  // return (dataset ? dataset : rdf.dataset()).import(quadStream).then(resolveFunc).catch(rejectFunc)
  return await this.loadTurtleFileIntoDatasetPromise(filename, dataset).then(resolveFunc).catch(rejectFunc)
}

/**
 * Saves a dataset to the specified file as RDF (uses Turtle if no media type provided).
 *
 * @param dataset
 * @param filename
 * @param resolveFunc
 * @param rejectFunc
 */
module.exports.saveDatasetToFile = (dataset, filename, resolveFunc, rejectFunc, mediaType = 'text/turtle', encoding = 'utf-8') => {
  const fileStream = fs.createWriteStream(filename, { encoding: encoding })

  const serializer = rdfFormats.serializers[ mediaType ]
  const quadStream = serializer.import(dataset.toStream())
  quadStream.pipe(fileStream)
}

/**
 * Serializes the specified dataset using the specified media type (e.g. JSON-LD, Turtle, etc.) and returns it as a
...
/**
 * Convenience method for debugging - writes to console the specified quad or quads. Works asynchronously, so you need
 * to invoke it with 'await'.
 *
...
module.exports.console = async (quads, message = '', mediaType = 'text/turtle', encoding = 'utf-8') => {
  const result = await this.datasetToString(Array.isArray(quads) ? rdf.dataset().addAll(quads) : rdf.dataset().add(quads))
...
/**
 * Extract the local name from the specified IRI (can be a primitive string or a NamedNode).
 *
 * @param stringOrNamedNode The IRI to extract from.
 * @returns {string}
 */
module.exports.extractIriLocalName = (stringOrNamedNode) => {
  const iri = module.exports.isString(stringOrNamedNode) ? stringOrNamedNode : stringOrNamedNode.value
...
/**
 * Returns a newly minted IRI (NamedNode) that is relative to the specified IRI. If no relative reference provided,
 * then the original IRI is simply returned as-is.
 *
 * @param iri
 * @param relativePart
...
/**
 * Simply treat the value as an IRI if it starts with 'http://' or 'https://' (case-insensitive).
 *
 * @param value
 * @returns {boolean}
 */
...
/**
 * Validates that the specified value is a valid IRI - i.e. either an explicit NamedNode, or a string that starts with
 * a valid HTTP(s) scheme.
 *
 * NOTE: This method is highly restricted to use work with LIT-specific classes, i.e. classes that *MUST* implement
 * specific methods, since we need to invoke these methods for error conditions.
 *
...
module.exports.validateIri = (value, rdfComponent, litContextObject) => {
  ...
  if (this.isString(value) && !this.isStringIri(value)) {
    litContextObject.reset()
  ...
  return this.isString(value) ? rdf.namedNode(value) : value
}
...
/**
 * Simple method that tries to offer an explanation if we detect mismatched IRI values.
 * NOTE: We do not check for equality here, we expect to be called in the case of a mismatch.
 *
 * This happened where the developer correctly passed a NamedNode IRI for one value, but a String for the other
 * (meaning the two values 'look' identical (e.g. if you used console.log()), but in fact were completely different
 * objects). Rather than try and handle this situation (i.e. by only comparing value.toString()) we explicitly want to
 * flag this situation as an error, as it could easily lead to all kinds of problems later, since IRI's and String are
 * two very different concepts.
 *
 * @param messagePrefix A message to prefix our returning message - intended to provide application-level context.
